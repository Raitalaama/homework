...
def save_q(file_name, session):
    saver = tf.train.Saver()
    print('writing trained Q to file', file_name)
...
def learn(env,
    ...
    ######
    q_func_network = q_func(obs_t_float,num_actions,scope="q_func",reuse=False)
    target_q_func_network = q_func(obs_tp1_float,num_actions,scope="target_q_func",reuse=False)
    
    q_func_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope ='q_func')
    target_q_func_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope ='target_q_func')
    
    
    selected_action_q = tf.reduce_sum(tf.one_hot(act_t_ph,depth=num_actions)*q_func_network,axis=1)
    
    if double_q_learning:
        double_q_func_network = q_func(obs_tp1_float,num_actions,scope="q_func",reuse=True)

        selected_target_action = tf.one_hot(tf.argmax(double_q_func_network,axis=1),depth=num_actions)
        target_q_value = tf.reduce_sum(target_q_func_network*selected_target_action,axis=1)
        ...
        y = rew_t_ph +done_mask_ph*gamma*tf.reduce_max(target_q_func_network,axis=1) 
    ...
    logdir = logdir[:-1] 
    iteration = 1 
    while os.path.exists(os.path.join('data/',(logdir+'/'+str(iteration)))):
        iteration+=1              
    ...
    try:
        for t in itertools.count():
            ...
            if not model_initialized or np.random.rand(1) < epsilon:
                ...
                action_values= session.run(q_func_network, feed_dict={obs_t_float : encoded_obsv[None]})
            ...
            if (t > learning_starts and
                ...
                train_feed_dict = {learning_rate: optimizer_spec.lr_schedule.value(t),rew_t_ph: rew_batch ,obs_t_ph: obs_t_batch,obs_tp1_ph: obs_tp1_batch,act_t_ph: act_batch, done_mask_ph: inverted_done_mask}
                ...
                _,loss_value,= session.run([train_fn,total_error], feed_dict=train_feed_dict)
                num_param_updates+=1
            ...
            if t % LOG_EVERY_N_STEPS == 0 and model_initialized:
               ...
               # print(qtn[0])
                ...
                logz.log_tabular("WallTime", time.strftime("%d.%m.%y %H:%M:%S", time.localtime())
                                 )
